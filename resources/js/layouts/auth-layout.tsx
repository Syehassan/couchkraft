...
export default function AuthLayout({
    ...
    const { logoLight, logoDark, themeColor, customColor, favicon, themeMode, footerText } = useBrand();
                ...
                >
                    ...
                    <div className="text-center mb-8">
                        <div className="relative inline-block px-6 rounded-xl">
                            {currentLogo ? (
                                ...
                                <h2 className="text-2xl font-bold" style={{ color: primaryColor }}>Couchkraft</h2>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="text-center mt-6">
                        <div className="inline-flex items-center space-x-2 bg-white dark:bg-slate-800/80 backdrop-blur-sm rounded-md px-4 py-2 border border-gray-200 dark:border-slate-700">
                            <p className="text-sm text-gray-500 dark:text-gray-400">{footerText || `© ${new Date().getFullYear()} COUCHKRAFT`}</p>
                        </div>
                    </div>
                </div>
            </div>
