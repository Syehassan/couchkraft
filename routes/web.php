...
// RAW STORAGE CHECK: Bypass routing entirely for image storage
if (str_starts_with(request()->path(), 'imagestorage/')) {
    $path = substr(request()->path(), 13); // remove 'imagestorage/'
    ...
    if (Illuminate\Support\Facades\Storage::disk('public')->exists($path)) {
        ...
        $contentType = $mimeTypes[strtolower($extension)] ?? mime_content_type($file);
