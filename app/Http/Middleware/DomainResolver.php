...
class DomainResolver
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        // Comprehensive list of admin/system routes to exclude from Store Resolution
        // This ensures these paths are always handled by the main application logic
        if ($request->is([
            'dashboard*', 'admin*', 'seller*', 'password*', 'logout', 
            'verify-email*', 'email*', 'confirm-password*',
            'plan-requests*', 'plan-orders*', 'plans*', 
            'products*', 'categories*', 'tax*', 'coupon-system*', 'shipping*', 
            'customers*', 'orders*', 'blogs*', 'reviews*', 'newsletter-subscribers*', 
            'express-checkout*', 'analytics*', 'payment-gateways*', 'pos*', 
            'ai-templates*', 'webhooks*', 'media-library*', 'examples*', 
            'permissions*', 'roles*', 'users*', 'companies*', 'coupons*', 
            'referral*', 'currencies*', 'manage-language*', 'language*', 'landing-page*',
            'impersonate*', 'switch-store*', 'user*', 'api*',
            'store/*', 'stores*',
            // Exclude Settings & Profile Routes
            'profile*', 'settings*', 'payment-methods', 'enabled-payment-methods',
            'email-notification-settings-save', 'stripe-payment', 'paypal-payment', 'bank-payment',
            // Exclude Auth Routes
            'register', 'forgot-password', 'reset-password',
            // Exclude System/Other Routes
            'custom-pages*', 'email-templates*', 'notification-templates*', 'store-coupons*',
            'countries*', 'states*', 'cities*', 'storage*', 'imagestorage*'
        ])) {
            return $next($request);
        }
