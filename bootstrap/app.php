...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withMiddleware(function (Middleware $middleware) {
        ...
        $middleware->redirectUsersTo(function ($request) {
            // If accessing customer guard (store login)
            if (\Illuminate\Support\Facades\Auth::guard('customer')->check()) {
                // If we have a resolved store, use it
                if ($request->attributes->has('resolved_store')) {
                    $store = $request->attributes->get('resolved_store');
                    return route('store.home', $store->slug);
                }
                
                // If we have a store slug in route
                $storeSlug = $request->route('storeSlug');
                if ($storeSlug) {
                    return route('store.home', $storeSlug);
                }
                
                // Fallback to home if no store context found (should cover edge cases)
                return route('store.demo'); // Or a generic landing
            }
            
            // Web guard (Admin/Seller) always goes to dashboard
